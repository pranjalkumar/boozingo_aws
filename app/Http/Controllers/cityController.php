...
class cityController extends Controller
{
    ...
    public function showCityDetail($city)
    {
        ...
        if($city_id_count==1)
        {
            ...
            select('beer_shop_name','beer_shop_address','beer_shop_not_working_day','beer_shop_cost','beer_shop_time','beer_shop_contact','beer_shop_images')->get()->toArray();
            ...
            select('liquor_shop_name','liquor_shop_address','liquor_shop_not_working_day','liquor_shop_cost','liquor_shop_time','liquor_shop_contact','liquor_shop_images')->get()->toArray();

            $lounges_list=DB::table('lounges')->where('city_id','=',$city_id)->join('lounge_images','lounges.id','=','lounge_images.lounge_id')->select('lounge_name','lounge_address',
                'lounge_cost','lounge_not_working_day','lounge_time','lounge_contact','lounge_images')->get()->toArray();

            $model_shop_list=DB::table('model_shops')->where('city_id','=',$city_id)->join('model_shop_images','model_shops.id','=','model_shop_images.model_shop_id')->select('model_shop_name',
            ...
            $night_clubs_list=DB::table('night_clubs')->where('city_id','=',$city_id)->join('night_club_images','night_clubs.id','=','night_club_images.night_club_id')->select('night_club_name',
            ...
            $pubs_list=DB::table('pubs')->where('city_id','=',$city_id)->join('pub_images','pubs.id','=','pub_images.pub_id')->select('pub_name','pub_address','pub_not_working_day','pub_cost','pub_time','pub_contact',
