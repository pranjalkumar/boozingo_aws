...
class detailedInformationController extends Controller
{
    ...
    public function detail($type,$type_id){
            ...
            {
                $data=DB::table('beer_shops')->where('id','=',$type_id)->select('beer_shop_name','beer_shop_address','beer_shop_details','beer_shop_geolocation','beer_shop_time',
                ...
                $result=json_encode(array('beer_shop'=>$data,'beer_shop_images'=>$data_images));
                if(count($data)==0)
                {
                    return 'Sorry!! There is no result to display';
                }
            ...
            {
                $data=DB::table('liquor_shops')->where('id','=',$type_id)->select('liquor_shop_name','liquor_shop_address','liquor_shop_details','liquor_shop_geolocation',
                ...
                $result=json_encode(array('liquor_shop'=>$data_images,'liquor_shop_images'=>$data_images));
                if(count($data)==0)
                {
                    return 'Sorry!! There is no result to display';
                }
            ...
            elseif ($type=='lounge')
            {
                ...
                $result=json_encode(array('lounge'=>$data,'lounge_images'=>$data_images));
                if(count($data)==0)
                {
                    return 'Sorry!! There is no result to display';
                }
            ...
            {
                $data=DB::table('model_shops')->where('id','=',$type_id)->select('model_shop_name','model_shop_address','model_shop_details','model_shop_geolocation',
                ...
                $result=json_encode(array('model_shop'=>$data,'model_shop_images'=>$data_images));
                if(count($data)==0)
                {
                    return 'Sorry!! There is no result to display';
                }
